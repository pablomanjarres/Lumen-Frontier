...
export default function BottomNavBar({ onNavigate, isVisible = true }: BottomNavBarProps) {
  const navItems = [
    ...
    // Time Tracker — a stopwatch (crown + stem + hand), distinct from Pomo's plain clock and Music's note.
    { id: 'track', label: 'Track', icon: 'M10 4h4M12 7V4M12 21a7 7 0 100-14 7 7 0 000 14zM12 10.5V14l2.5 1.5' },
  ...
  return (
    <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-50 transition-all duration-300 ease-in-out ${
      ...
      <div className="flex items-center gap-2 bg-surface-raised/80 backdrop-blur-xl border border-hairline rounded-card px-3 py-2 shadow-soft-lg">
        {navItems.map((item) => (
          <div key={item.id} className="group relative">
            {/* Tooltip — hover + keyboard focus */}
            <div className="pointer-events-none absolute -top-10 left-1/2 -translate-x-1/2 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
              <div className="relative bg-surface-overlay text-secondary text-xs font-medium px-3 py-1 rounded-control whitespace-nowrap border border-hairline shadow-soft">
                {item.label}
                <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-surface-overlay rotate-45 border-r border-b border-hairline" />
              </div>
            </div>
            ...
            <button
              ...
              className="p-3 rounded-control transition-colors hover:bg-brass-500/10 focus-visible:outline-none focus-visible:shadow-focus"
            >
              <svg
                className="w-6 h-6 text-secondary transition-colors group-hover:text-brass-300"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
